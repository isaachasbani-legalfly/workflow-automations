# Country Retroactive Enrichment v2.2 — Architecture
...
flowchart TD
    ...
    ParseAmpLinkedIn --> CheckAmpLinkedIn{Got Country from LinkedIn?}
...
**Enrichment source**: `"Website"` (from scrape) or `"Web Search"` (from DuckDuckGo) or `"Amplemarket"` (from LinkedIn lookup)
...
- **Jina replaces Gemini internal knowledge**: v2.0's blind pass hallucinated (classified "dewitlawoffice" as Netherlands instead of Belgium). Jina provides real web content, eliminating LLM guessing.
...
| v2-amp-linkedin | Amplemarket LinkedIn Lookup | httpRequest |
| v2-parse-amp-linkedin | Parse LinkedIn Amplemarket Country | code |
| v2-check-amp-linkedin | Check LinkedIn Amplemarket Got Country | if |
